...
public class HeapSort {
	...
	/** 
	* Defines the heap sort algorithm, an algorithm that places an array's data into a heap and then removes
	* each heap item and moves it back into the array in a sorted order. This algorithm has an O(n log(n)) complexity
	* 
	...
	*/
	...
	private static <T extends Comparable<T>> void buildHeap(T[] table) { 
		...
		while (n < table.length) {
			...
			int child = n - 1; int parent = (child - 1) / 2; // Find parent. 
			while (parent >= 0
